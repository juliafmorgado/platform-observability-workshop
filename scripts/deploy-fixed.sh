...
# ── 3. Deploy fixed manifests ────────────────────────────────────────────────
step "Deploying fixed manifests to cluster"
kubectl apply -f "${REPO_ROOT}/k8s/fixed/namespace.yaml"
...
kubectl apply -f "${REPO_ROOT}/k8s/fixed/otel-collector.yaml"
kubectl apply -f "${REPO_ROOT}/k8s/fixed/otel-platform-config.yaml"
kubectl apply -f "${REPO_ROOT}/k8s/fixed/inventory-service.yaml"
kubectl apply -f "${REPO_ROOT}/k8s/fixed/order-service.yaml"
kubectl apply -f "${REPO_ROOT}/k8s/fixed/shipping-service.yaml"
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  Meridian is running — fixed state deployed      ║${NC}"
echo -e "${GREEN}╠══════════════════════════════════════════════════╣${NC}"
